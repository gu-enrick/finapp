...
// Mock do requireAuth — simula usuário autenticado em todos os testes
jest.mock("../auth", () => ({
...
beforeAll(async () => {
  ...
  const { rows } = await pool.query(
    "INSERT INTO categories (name, type, color, user_id) VALUES ('Teste', 'expense', '#ef4444', $1) RETURNING id",
    [TEST_USER]
  );
...
describe("Transactions API", () => {
  ...
  test("POST /api/transactions cria uma transação válida", async () => {
    const res = await request(app)
      ...
      .send({ type: "expense", amount: 50.5, description: "Teste", category_id: categoryId, date: "2026-06-15" });
  ...
  test("POST /api/transactions rejeita amount negativo", async () => {
    const res = await request(app).post("/api/transactions").send({ type: "expense", amount: -10, date: "2026-06-15" });
    expect(res.status).toBe(400);
  });

  test("POST /api/transactions rejeita data inválida", async () => {
    const res = await request(app).post("/api/transactions").send({ type: "expense", amount: 10, date: "9999-99-99" });
    expect(res.status).toBe(400);
  });

  test("POST /api/transactions rejeita type inválido", async () => {
    const res = await request(app).post("/api/transactions").send({ type: "invalido", amount: 10, date: "2026-06-15" });
    expect(res.status).toBe(400);
  });
  ...
  test("GET /api/transactions retorna a transação criada", async () => {
    ...
    expect(found.description).toBe("Teste");
  });

  test("GET /api/transactions filtra por categoria", async () => {
    const res = await request(app).get("/api/transactions").query({ category_id: categoryId });
    expect(res.status).toBe(200);
    expect(res.body.data.every(t => t.category_id === categoryId)).toBe(true);
  });

  test("PUT /api/transactions/:id atualiza a transação", async () => {
    const res = await request(app).put(`/api/transactions/${createdId}`)
      .send({ type: "expense", amount: 99.9, description: "Atualizado", category_id: categoryId, date: "2026-06-16" });
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ ok: true });
  });

  test("PUT /api/transactions/:id com ID inexistente retorna 404", async () => {
    const res = await request(app).put("/api/transactions/999999")
      .send({ type: "expense", amount: 10, date: "2026-06-15" });
    expect(res.status).toBe(404);
  });

  test("PATCH /api/transactions/:id/confirm confirma transação prevista", async () => {
    const created = await request(app).post("/api/transactions")
...
describe("Reports API", () => {
  test("GET /api/reports/summary retorna estrutura correta", async () => {
    const res = await request(app).get("/api/reports/summary").query({ start: "2026-01-01", end: "2026-12-31" });
    expect(res.status).toBe(200);
