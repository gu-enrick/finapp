...
beforeAll(async () => {
  ...
  const { rows } = await pool.query(
    "INSERT INTO categories (name, type, color, user_id) VALUES ('Delivery', 'expense', '#f97316', $1) RETURNING id",
    [TEST_USER]
  );
  ...
  await pool.query(
    "INSERT INTO transactions (type, amount, category_id, date, is_confirmed, user_id) VALUES ('income', 3000, NULL, $1, TRUE, $2)",
    [today, TEST_USER]
  );
  await pool.query(
    "INSERT INTO transactions (type, amount, category_id, date, is_confirmed, user_id) VALUES ('expense', 200, $1, $2, TRUE, $3)",
    [categoryId, today, TEST_USER]
  );
  await pool.query(
    "INSERT INTO transactions (type, amount, category_id, date, is_confirmed, user_id) VALUES ('expense', 500, NULL, $1, TRUE, $2)",
    [today, TEST_USER]
  );
});
...
describe("Goals API", () => {
  ...
  test("POST /api/goals cria meta de economia", async () => {
    const res = await request(app).post("/api/goals").send({ kind: "savings", label: "Reserva", amount: 1000 });
  ...
  test("POST /api/goals rejeita category_limit sem category_id", async () => {
    const res = await request(app).post("/api/goals").send({ kind: "category_limit", label: "Sem cat", amount: 100 });
    expect(res.status).toBe(400);
  });

  test("POST /api/goals cria meta de saldo", async () => {
    const res = await request(app).post("/api/goals").send({ kind: "balance", label: "Saldo mínimo", amount: 500 });
    expect(res.status).toBe(201);
    balanceId = res.body.id;
  });

  test("POST /api/goals rejeita kind inválido", async () => {
    const res = await request(app).post("/api/goals").send({ kind: "invalido", label: "Teste", amount: 100 });
    expect(res.status).toBe(400);
  });

  test("POST /api/goals rejeita valor negativo", async () => {
    const res = await request(app).post("/api/goals").send({ kind: "savings", label: "Negativa", amount: -50 });
    expect(res.status).toBe(400);
  });

  test("GET /api/goals calcula progresso de limite por categoria", async () => {
    const res = await request(app).get("/api/goals");
    const limit = res.body.find(g => g.id === limitId);
    expect(limit).toBeDefined();
    expect(limit.current).toBe(200);
    expect(limit.percent).toBeCloseTo((200 / 350) * 100, 1);
    expect(limit.exceeded).toBe(false);
  });

  test("GET /api/goals calcula meta de economia corretamente", async () => {
    const res = await request(app).get("/api/goals");
    const savings = res.body.find(g => g.id === savingsId);
    expect(savings.current).toBe(2300); // 3000 - 200 - 500
  });

  test("GET /api/goals marca exceeded=true quando limite estourado", async () => {
    const today = new Date().toISOString().slice(0, 10);
    await pool.query(
      "INSERT INTO transactions (type, amount, category_id, date, is_confirmed, user_id) VALUES ('expense', 999, $1, $2, TRUE, $3)",
      [categoryId, today, TEST_USER]
    );
    const res = await request(app).get("/api/goals");
    const limit = res.body.find(g => g.id === limitId);
  ...
  test("PUT /api/goals/:id atualiza meta", async () => {
    const res = await request(app).put(`/api/goals/${savingsId}`)
  ...
  test("PUT /api/goals/:id com ID inexistente retorna 404", async () => {
    const res = await request(app).put("/api/goals/999999")
