...
export default function TransactionModal({
  ...
  const shift = (unit, amount) =>
    set("date", shiftDate(form.date, unit, amount));

  const handleSubmit = () => {
    const description = normalizeText(form.description);
    if (!isPositiveNumber(form.amount))
      return toast.error(VALIDATION_MESSAGES.invalidAmount);
    if (!isValidDateString(form.date))
      return toast.error(VALIDATION_MESSAGES.invalidDate);
    onSave({
      ...form,
      description,
  ...
  const sourceCategories =
    allCategories.length > 0 ? allCategories : categories;
  const filtered = sourceCategories.filter(
    (c) => c.type === form.type || c.type === "both",
  );

  return createPortal(
    /* A mudança principal está na linha abaixo: trocado "items-end sm:items-center" por apenas "items-center" */
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 w-full max-w-md shadow-xl border border-gray-100 dark:border-gray-800 max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <input
                ...
                className="w-full sm:w-auto flex-1 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
              />
            </div>
            <div className="flex gap-1.5 mt-2 flex-wrap">
              ...
              <button
                onClick={() => set("date", getLocalToday())}
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
              placeholder="Ex: Conta de luz de maio"
            />
          </div>
        </div>
