    ...
    try {
      if (mode === "login") {
        ...
        if (data.user && !data.user.confirmed_at) {
          setMessage(
            "Confirmação enviada para o seu e-mail. Verifique a caixa de entrada.",
          );
          setMode("login");
        } else {
          onLogin(data.user);
...
export default function App() {
  ...
  const [consented, setConsented] = useState(
    () => localStorage.getItem("finvolt_consent") === "true",
  );
      ...
      {isMobile ? (
        <div className="flex-1 min-h-0 relative w-full overflow-hidden">
          <div
            ...
            onTouchStart={(e) => {
              prevPageRef.current = e.touches[0].clientX;
            }}
            onTouchEnd={(e) => {
          ...
          >
            {MOBILE_PRIMARY.map((id) => (
              <div
                key={id}
                style={{ width: `${100 / MOBILE_PRIMARY.length}%` }}
                className="h-full overflow-y-auto px-4 pt-4 pb-24 shrink-0"
              >
