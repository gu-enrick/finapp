      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-sm font-medium text-gray-800 dark:text-gray-100">{goal.label}</span>
          ...
          <div className="mt-2 flex items-end justify-between">
            ...
            <span className={`text-sm font-medium ${statusColor}`}>{percent.toFixed(0)}%</span>
          </div>
...
export default function Goals({ categories }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) { await updateGoal(editing, data); toast.success("Meta atualizada"); setEditing(null); }
