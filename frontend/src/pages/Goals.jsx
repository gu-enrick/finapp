...
const KINDS = {
  ...
  category_limit: {
    ...
    desc: "Teto de gasto numa categoria",
  },
...
function GoalCard({ goal, onDelete, onEdit }) {
  ...
  const statusText = exceeded
    ...
    : percent >= 90
      ...
      : isLimit
        ? `${fmt(goal.amount - goal.current)} restante`
        : `${fmt(goal.current)} de ${fmt(goal.amount)}`;

  return (
    <div
      className={`bg-white dark:bg-gray-900 rounded-xl p-4 shadow-sm border ${exceeded ? "border-red-200 dark:border-red-800" : "border-gray-100 dark:border-gray-800"}`}
    >
          ...
          {goal.category_name && (
            <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 shrink-0">
              <span
                ...
                style={{ background: goal.category_color }}
              />
              {goal.category_name}
            </span>
          )}
      ...
      <div className="flex items-end justify-between mt-2">
        ...
        <span className={`text-sm font-medium ${statusColor} shrink-0 ml-2`}>
          {percent.toFixed(0)}%
        </span>
      </div>
...
export default function Goals({ categories }) {
  ...
  const [period, setPeriod] = useState(() => {
    ...
    // Pega o último dia do mês atual (ex: 31, 30, 28)
    const lastDay = new Date(year, n.getMonth() + 1, 0).getDate();
  ...
  const handleSave = async () => {
    if (!form.label || !form.amount)
      return toast.error("Label e valor são obrigatórios");
    if (form.kind === "category_limit" && !form.category_id)
        ...
        setGoals((prev) =>
          prev.map((g) => {
            if (g.id === editing) {
              ...
              const newPercent =
                newAmount > 0 ? (g.current / newAmount) * 100 : 0;
              const newExceeded =
                data.kind === "category_limit" && g.current > newAmount;
              ...
              return {
                ...
                category_name: cat ? cat.name : g.category_name,
                category_color: cat ? cat.color : g.category_color,
              };
            }
            return g;
          }),
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div
            className={`flex gap-2 ${isMobile ? "flex-col w-full" : "items-center"}`}
          >
